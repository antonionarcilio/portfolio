...
import { CARD_STAGGER_STEP, cardVariants } from '@/features/gamified/animations';
import { useA11y } from '@/features/gamified/contexts/a11y-context';
import { useSkillProjects } from '@/features/gamified/hooks/use-skill-projects';
...
const PANEL_LS_KEY = 'gamified:skillmap:panel';
      ...
      techTree.map((c, i) => {
        ...
        const icon = c.iconUrl ? (
          <SvgIcon
            ...
            size={15}
            color="var(--cyan)"
            className={isEgg ? '!cursor-gamified-help' : undefined}
          />
        ) : undefined;
        return {
        ...
        {showHint && (
          <Tooltip content={t('hint')} placement="bottom">
            ...
            >
              <ShimmerStatus
                text={t('extraContent')}
                className="cv-shimmer-hint text-[10px] cursor-gamified-help normal-case"
              />
            </motion.div>
          </Tooltip>
        )}
