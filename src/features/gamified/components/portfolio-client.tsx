...
export default function PortfolioClient({ data }: { data: PortfolioData }) {
  ...
  useEffect(() => {
    document.documentElement.classList.add('cv-page-scroll', 'cv-gamified-root');
    return () => {
      document.documentElement.classList.remove('cv-page-scroll', 'cv-gamified-root');
    };
  }, []);

  return (
    <div className="cv-gamified-wrapper bg-cv-wrapper text-cv-text font-cv-mono text-[14px] leading-[1.5] tracking-[0.02em] min-h-screen">
