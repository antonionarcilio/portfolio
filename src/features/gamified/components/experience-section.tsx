...
import { formatYearRange } from '@/features/gamified/utils/format-experience-date-range';
...
export function ExperienceSection({
  ...
  const t = useTranslations('gamified.experience');
        ...
        <ScrollList maxHeight={170} maxHeightMobile={300} itemCount={items.length} hideScrollHint hideScrollbar>
          {items.map((item, i) => (
            <AnimatedCard
              key={item.company}
              index={i}
              className="relative group border border-cv-border bg-cv-panel px-5 py-[18px] border-l-2 border-l-cv-border cursor-gamified-pointer outline-none focus-visible:outline-none"
                ...
                <div className="flex justify-between items-center">
                  ...
                  <Tooltip title={t('extraTooltip')}>
                    <div>
                      <ShimmerStatus
                        text={t('extraContent')}
                        className="cv-shimmer-hint text-[10px] cursor-gamified-help"
                      />
                    </div>
                  </Tooltip>
                </div>
