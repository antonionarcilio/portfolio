    ...
    <ModalBase open={show} onClose={onClose} portalId="gamer-portal-root">
      {({ floatingRef, floatingProps, panelStyles }) => (
        <div
          ref={floatingRef}
          style={panelStyles}
          className="relative max-w-[640px] w-full bg-cv-panel border border-cv-cyan shadow-[inset_0_0_30px_rgba(43,214,255,0.05),0_8px_24px_rgba(0,0,0,0.5),0_0_18px_rgba(43,214,255,0.18)] outline-none"
          {...floatingProps}
        >
          ...
          <div className="cv-scroll overflow-x-hidden max-h-[86vh] px-[30px] pt-[28px] pb-[26px]">
            <div className="absolute top-3 right-[14px]">
              <Tooltip key={String(show)} content="Fechar" placement="left" className="!z-[9999]">
                <motion.button
                  className="bg-transparent border border-cv-border text-cv-cyan font-cv-mono text-[16px] w-7 h-7 flex items-center justify-center cursor-pointer leading-none outline-none"
                  onClick={onClose}
            ...
            <div className="text-cv-cyan text-[10px] tracking-[0.28em] uppercase mb-[14px]">{'// Project_Record'}</div>
            <h2 className="text-[22px] text-cv-text m-0 mb-1 tracking-[0.04em]">{data.projectName}</h2>
            <div className="text-[14px] text-cv-cyan tracking-[0.08em]">{data.company}</div>
