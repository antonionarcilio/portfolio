...
import { SwitcherLanguage } from './styles';

const LocaleSwitcher = () => {
  const router = useRouter();
  const { locale } = router;
  const { currentLocale, handleToggle } = useContext(LocaleContext);

  const translate:I18nTypes = locale === 'en-US' ? enUS : ptBR;

  // 🍪 Criando cookies
  useEffect(() => {
    Cookies.set('NEXT_LOCALE', String(currentLocale));
  }, [currentLocale]);
  ...
  return (
    <>
      <SwitcherLanguage
        ...
        id="switchLanguage"
        data-tip={`${translate.tooltip.locale_switcher.text}`}
      >
        {locale === 'en-US' ? (
          ...
          <svg viewBox="0 0 512 512" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M256 512C397.385 512 512 397.385 512 256C512 114.615 397.385 0 256 0C114.615 0 0 114.615 0 256C0 397.385 114.615 512 256 512Z" fill="#05AB41" />
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  return {
    props: {
      currentLocale: String(NEXT_LOCALE),
    },
  };
};
