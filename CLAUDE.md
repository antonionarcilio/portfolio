...
- **Decision rule — CSS vs. inline Tailwind**: if a declaration does not depend on a media/container query, a pseudo-element, a relational selector (`+`, `:not()`, compound selectors), `@keyframes`, or a custom property that changes by theme/JS state, it has no reason to live in `styles.css` — write it inline as Tailwind utility classes using the tokens already exposed via `@theme inline` (`text-minimalist-*`, `color-minimalist-*`, `font-weight-minimalist-*`, etc.). This applies even to a single declaration or a short block (e.g. `position: absolute; inset: 0; z-index: 4; background: var(--minimalist-background)` is `absolute inset-0 z-4 bg-minimalist-background` in JSX, not a BEM class with a 4-line CSS rule).
- Interactions that are purely CSS pseudo-classes (`hover`, `focus-visible`, `disabled`) use the native Tailwind variant (`hover:`, `focus-visible:`, `disabled:`) directly in `className` (optionally via `cva`), never a hand-maintained BEM modifier (`--hover`, `--focus`). Reserve manual modifier classes for state that only exists in JS/React (`--selected`, `--current`, `--active`) and cannot be expressed as a CSS pseudo-class.
- The same set of Tailwind utility classes repeated in 2+ components follows the "Shared animation values" rule below: extract it to a named constant (a `cva` definition or exported class-name string) instead of copy-pasting the full `className` in each file.
...
### Responsiveness — named breakpoint tokens

- **Breakpoints are centralized tokens** in a `@theme` block (never `inline`) inside the feature's `styles.css`. Each feature namespaces its own tokens (e.g. `--breakpoint-mobile: 670px`); Tailwind v4 merges all `@theme` blocks globally, so keep token names unique across features (`minimalist` must not redeclare `--breakpoint-cv`, defined in `gamified/styles.css:22`).
- **Decision rule**: a Tailwind utility + breakpoint → `className` with a *named variant* (`max-mobile:`, `min-tablet:`). Custom CSS + breakpoint → a `@media (width <= theme(--breakpoint-*))` rule tied to a stable BEM class in `src/features/<feature>/styles.css`.
- **Never** write arbitrary breakpoint literals (`max-[Npx]:`, `min-[Npx]:`, `max-cv:`, `min-cv:`) in a component's `className`, and **never** hardcode the breakpoint value (`670px`, `880px`, `55rem`) in `@media` — always `theme(--breakpoint-*)` in the CSS or the named variant in JSX. This applies even to a single isolated class.
- **No duplication**: each responsive rule lives in exactly one place — JSX (named variant) *or* CSS (`@media` + BEM class), never both. Do not create a CSS media query that mirrors a `max-*:` variant already present in the JSX, and vice-versa.
- **`.css` keeps**: `@container` queries, a11y `html.a11y-*` rules, pseudo-elements, complex selectors, gradients/masks, overflow/scroll geometry, keyframes, and the `@theme`/`@theme inline` token blocks.
- **Guardrail**: a feature `styles.css` past ~800 lines is a signal that layout is leaking into CSS — migrate simple layout-responsive rules to named JSX variants or CVA variants.
- When a style depends on an accessibility state already reflected as a class on `<html>` (e.g. `html.a11y-upscale` for the 1.2× zoom), **don't** replicate that condition with a JS ternary inside `className` (`opts.upscale ? 'a' : 'b'`). Instead, use a single static class in the JSX and resolve the difference in CSS with `html.a11y-upscale .my-class { … }` / `html:not(.a11y-upscale) .my-class { … }` — see the "two thresholds" pattern already used in `minimalist/styles.css`.
